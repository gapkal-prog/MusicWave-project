...
- در ناوبری نرم پخش‌کنندهٔ سراسری (`persistentNav`)، بلوک‌های تعاملی هسته (منوی همبرگر و جست‌وجوی بازشونده) ممکن است پس از تعویض `<body>` دوباره hydrate نشوند؛ این محدودیت پیش از این تغییر هم وجود داشت و به Interactivity API هسته مربوط است.
