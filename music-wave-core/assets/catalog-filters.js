...
(function (config) {
	...
	if (!config || !window.fetch || !window.DOMParser || !window.history || !history.pushState) {
		return;
	}

	var REGION_SELECTORS = ['form.mw-catalog-filters', '.mw-catalog-results', '.wp-block-query'];
	var supportsSwap = !!document.querySelector('.wp-block-query');
	var inFlight = null;

	if (!document.querySelector('form.mw-catalog-filters') || !supportsSwap) {
		return;
	}
	...
	function setLoading(loading) {
		...
		if (loading) {
			...
			if (button) {
				button.setAttribute('data-mw-original-label', button.textContent);
	...
	function buildSubmitUrl(form) {
		...
		var base = String(form.action || window.location.href).split(/[?#]/)[0];
	...
	document.addEventListener('submit', function (event) {
		...
		if (!form || !form.matches || !form.matches('form.mw-catalog-filters')) {
			return;
		}
	...
	document.addEventListener('click', function (event) {
		var link = event.target && event.target.closest
			? event.target.closest('.mw-catalog-results a, .wp-block-query-pagination a')
			: null;
		if (!link) {
			return;
		}
		...
		event.preventDefault();
		loadCatalogUrl(new window.URL(href, window.location.href).href, true);
	});
