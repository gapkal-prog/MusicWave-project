...
(function (config) {
	...
	function setup(input) {
		...
		var listId = 'mw-catalog-suggest-list-' + Math.random().toString(36).slice(2, 8);
		...
		function announce(count) {
			...
			status.textContent = (config.resultsCount || '%d').replace('%d', String(count));
		}
		...
		function render(suggestions) {
			...
			suggestions.forEach(function (suggestion, index) {
				...
				type.textContent = config.typeLabels && config.typeLabels[suggestion.type] ? config.typeLabels[suggestion.type] : suggestion.type;
		...
		var query = debounce(function (term) {
			...
			var url = config.endpoint + (config.endpoint.indexOf('?') > -1 ? '&' : '?') + 'term=' + window.encodeURIComponent(term);
			window
				.fetch(url, {
		...
		input.addEventListener('keydown', function (event) {
			...
			if ('Enter' === event.key && active > -1) {
				event.preventDefault();
				items[active].dispatchEvent(new window.MouseEvent('mousedown'));
				return;
			}
			if ('Escape' === event.key) {
				close();
	...
	function init() {
		var inputs = document.querySelectorAll('.mw-catalog-suggest input[type="search"]');
