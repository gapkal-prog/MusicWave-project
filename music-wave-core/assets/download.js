(function () {
	...
	function setQualities(container, fileKey) {
		...
		var file = files.filter(function (candidate) {
		...
		if (!file || !Array.isArray(file.qualities) || !qualitySelect || !button) {
			return;
		}
		...
		button.setAttribute('data-download-quality', qualitySelect.value || '');
	}

	function selectedQuality(button) {
		var row = button.closest('.mw-download-file-row');
		var container = button.closest('.mw-download-action') || button.parentNode;
		var qualitySelect = row ? row.querySelector('.mw-download-quality') : container.querySelector('.mw-download-quality');

		return qualitySelect ? qualitySelect.value : (button.getAttribute('data-download-quality') || '');
	}
	...
	function token(releaseId, quality, purpose) {
		...
		return window.wp.apiFetch({
			...
			method: 'POST',
			headers: settings.restNonce ? {'X-WP-Nonce': settings.restNonce} : {}
		});
	}

	function protectedUrl(route, releaseId, response) {
		var url = window.musicWaveDownload.restUrl + 'music-wave/v1/' + route + '/' + releaseId;
	...
	function setPlayButton(button, playing) {
		...
		var playLabel = button.getAttribute('data-play-label') || labels.playLabel || 'Play';
		var pauseLabel = button.getAttribute('data-pause-label') || labels.pauseLabel || 'Pause';
	...
	function resetPlayButtons(except) {
		document.querySelectorAll('.mw-secure-play-button').forEach(function (button) {
			if (button !== except) {
	...
	document.addEventListener('change', function (event) {
		...
		var qualitySelect = event.target.closest('.mw-download-file-row .mw-download-quality');
		if (!qualitySelect) {
			return;
		}
		var row = qualitySelect.closest('.mw-download-file-row');
		var playButton = row ? row.querySelector('.mw-secure-play-button') : null;
		var container = row ? row.closest('.mw-download-action') : null;
		var audio = container ? container.querySelector('.mw-secure-audio') : null;
		var option = qualitySelect.options[qualitySelect.selectedIndex];
		if (audio && !audio.paused) {
			audio.pause();
		}
		if (playButton) {
			playButton.disabled = !option || option.getAttribute('data-streamable') !== '1';
	...
	document.addEventListener('click', function (event) {
		...
		var releaseId = parseInt(button.getAttribute('data-release-id'), 10);
		var container = button.closest('.mw-download-action') || button.parentNode;
		...
		token(releaseId, quality, 'download')
			.then(function (response) {
				window.location.assign(protectedUrl('downloads', releaseId, response));
			})
			.catch(function (error) {
				if (status) {
					status.textContent = errorMessage(error, window.musicWaveDownload.errorMessage);
				}
			})
			.finally(function () {
	...
	document.addEventListener('click', function (event) {
		...
		var audio = container ? container.querySelector('.mw-secure-audio') : null;
		var status = container ? container.querySelector('.mw-download-status') : null;
		var releaseId = parseInt(button.getAttribute('data-release-id'), 10);
		var quality = selectedQuality(button);
		if (!audio || !status || !releaseId || !quality) {
			return;
		}
		if (audio.getAttribute('data-active-quality') === quality && !audio.paused) {
			audio.pause();
			return;
		}
		if (audio.getAttribute('data-active-quality') === quality && audio.src) {
		...
		token(releaseId, quality, 'stream')
			...
			.catch(function (error) {
				status.textContent = errorMessage(error, window.musicWaveDownload.streamError);
			})
			.finally(function () {
				button.disabled = false;
			});
	...
	document.addEventListener('play', function (event) {
		...
		var buttons = container ? container.querySelectorAll('.mw-secure-play-button') : [];
	...
	document.addEventListener('pause', function (event) {
		...
		var container = event.target.closest('.mw-download-action');
		if (container) {
			container.querySelectorAll('.mw-secure-play-button').forEach(function (button) {
				setPlayButton(button, false);
			});
		}
	}, true);
