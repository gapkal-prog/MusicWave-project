(function () {
	...
	function trackFromButton(button) {
		return {
			...
			previewLimit: parseInt(button.getAttribute('data-preview-limit'), 10) || 30,
			full: false,
			quality: '',
			limited: false
		};
	...
	function trackFromApi(item, meta) {
		...
		return {
			...
			title: item.title || '',
			artist: item.artist || '',
			image: item.image || (meta && meta.release ? meta.release.image : ''),
			link: item.link || '',
	...
	function fetchStreamToken(track) {
		return window.wp.apiFetch({
			path: '/music-wave/v1/releases/' + track.releaseId + '/download-token?quality=' + encodeURIComponent(track.quality) + '&purpose=stream',
			method: 'POST',
			headers: restHeaders()
		});
	}
	...
	function streamUrl(track, response) {
		...
		url += '?token=' + encodeURIComponent(response.token) + '&nonce=' + encodeURIComponent(response.nonce);
	...
	function initialize(player) {
		...
		var progress = player.querySelector('.mw-global-player__progress input');
		var time = player.querySelector('.mw-global-player__time');
		var volume = player.querySelector('input.mw-global-player__volume');
		var queueToggle = player.querySelector('.mw-global-player__queue-toggle');
		var queuePanel = player.querySelector('[data-mw-queue]');
		var queueList = player.querySelector('.mw-global-player__queue-list');
		var queueClose = player.querySelector('.mw-global-player__queue-close');
		var notice = player.querySelector('.mw-global-player__notice');
		var noticeMessage = player.querySelector('.mw-global-player__notice-message');
		var noticeCta = player.querySelector('.mw-global-player__notice-cta');
		var noticeLogin = player.querySelector('.mw-global-player__notice-login');
		var noticeClose = player.querySelector('.mw-global-player__notice-close');
		var queue = [];
		var queueMeta = null;
		var current = -1;
		var busy = false;
		...
		function pageQueue() {
			var tracks = [];
			document.querySelectorAll('.mw-preview-button[data-preview-url]').forEach(function (button) {
				var track = trackFromButton(button);
				if (track.url && !tracks.some(function (item) { return item.url === track.url; })) {
					tracks.push(track);
				}
			});
			return tracks;
		...
		function setLoading(loading) {
			busy = loading;
			toggle.textContent = loading ? '…' : (audio.paused ? '▶' : '❚❚');
		}

		function updateButtons() {
			var track = currentTrack();
			var playing = Boolean(track) && !audio.paused;
			document.querySelectorAll('.mw-preview-button').forEach(function (button) {
				var active = playing && track.url !== '' && button.getAttribute('data-preview-url') === track.url;
				button.setAttribute('aria-pressed', active ? 'true' : 'false');
				var icon = button.querySelector('.mw-preview-button__icon');
				if (icon) {
					icon.textContent = active ? '❚❚' : '▶';
				}
			});
			document.querySelectorAll('.mw-card-play[data-mw-release-id]').forEach(function (button) {
				var releaseId = parseInt(button.getAttribute('data-mw-release-id'), 10) || 0;
				var active = playing && releaseId > 0 && (track.rootId === releaseId || track.releaseId === releaseId);
				button.setAttribute('aria-pressed', active ? 'true' : 'false');
		...
		function showNotice(message, upsell) {
			...
			if (noticeCta) {
				var hasCta = Boolean(upsell && upsell.ctaUrl && upsell.ctaLabel);
			...
			if (noticeLogin) {
				var hasLogin = Boolean(upsell && upsell.loginUrl && upsell.loginLabel);
		...
		function announcePreviewMode() {
			...
			if (upsell.message) {
				message = message ? message + ' ' + upsell.message : upsell.message;
			}
			if (message) {
		...
		function mediaSession(track) {
			...
			navigator.mediaSession.setActionHandler('previoustrack', function () { select(current - 1, true); });
			navigator.mediaSession.setActionHandler('nexttrack', function () { select(current + 1, true); });
		}
		...
		function render() {
			...
			toggle.setAttribute('aria-label', audio.paused ? (labels.play || 'Play preview') : (labels.pause || 'Pause preview'));
		...
		function select(index, autoplay) {
			...
			loadSource(track)
				...
				.catch(function (error) {
					...
					setLoading(false);
					render();
					showNotice(errorMessage(error, labels.streamError), queueMeta ? queueMeta.upsell : null);
				});
		}
		...
		function startPreviewButton(button) {
			...
			var selected = tracks.findIndex(function (track) { return track.url === clicked.url; });
			var sameQueue = queue.length > 0 && queue.every(function (track) { return !track.full && track.url; });
			...
			if (current >= 0 && queue[current] && queue[current].url === clicked.url && sameQueue) {
				if (audio.paused) {
		...
		function releasePlaying(releaseId) {
			var track = currentTrack();
			return Boolean(track) && (track.rootId === releaseId || track.releaseId === releaseId);
		}

		function startRelease(button, forcedReleaseId) {
			var releaseId = forcedReleaseId || parseInt(button.getAttribute('data-mw-release-id'), 10) || 0;
			if (releaseId < 1) {
				return;
			}
			...
			fetchReleaseQueue(releaseId)
				.then(function (data) {
					...
					queue = tracks.map(function (item) { return trackFromApi(item, data); }).filter(function (track) {
		...
		document.addEventListener('click', function (event) {
			...
			var cardButton = event.target.closest('.mw-card-play[data-mw-release-id]');
			if (cardButton) {
				event.preventDefault();
		...
		previous.addEventListener('click', function () { select(current - 1, true); });
		next.addEventListener('click', function () { select(current + 1, true); });
		if (queueToggle) {
		...
		if (queueClose) {
			queueClose.addEventListener('click', function () { setQueueOpen(false); });
		}
		if (noticeClose) {
		...
		if (volume) {
			volume.addEventListener('input', function () {
				audio.volume = Math.min(100, Math.max(0, parseFloat(volume.value) || 0)) / 100;
			});
		}
		...
		audio.addEventListener('error', function () {
			...
			setLoading(false);
			showNotice(labels.streamError || labels.error, queueMeta ? queueMeta.upsell : null);
			render();
		});
		audio.addEventListener('timeupdate', function () {
			var duration = audio.duration || 0;
			var track = currentTrack();
			if (track && track.previewLimit > 0 && audio.currentTime >= track.previewLimit) {
				audio.pause();
				audio.currentTime = 0;
				track.limited = true;
				if (queueMeta && queueMeta.upsell && (queueMeta.upsell.message || queueMeta.upsell.previewNote)) {
					var upsell = queueMeta.upsell;
			...
			time.textContent = formatTime(audio.currentTime) + (duration ? ' / ' + formatTime(duration) : '');
		});
		...
		playerController = {
			playRelease: function (trigger, releaseId) {
	...
	document.addEventListener('click', function (event) {
		var button = event.target && event.target.closest ? event.target.closest('.mw-secure-play-button') : null;
		if (!button || button.disabled || !playerController) {
			return;
		}
		event.preventDefault();
		event.stopPropagation();
		document.querySelectorAll('.mw-secure-audio').forEach(function (candidate) {
			candidate.pause();
		});
		playerController.playRelease(button, parseInt(button.getAttribute('data-release-id'), 10) || 0);
	}, true);
	...
	function navExcluded(url) {
		if (url.searchParams.has('add-to-cart') || url.searchParams.has('wc-ajax') || url.searchParams.has('remove_item')) {
			return true;
		}
		var path = url.pathname;
		return /\/wp-(admin|login|json)/.test(path)
			|| /\/(cart|checkout)(\/|$)/.test(path)
			|| navFileHref.test(path);
	}

	function navShouldIntercept(link, event) {
		if (!navSupported() || event.defaultPrevented || event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) {
			return false;
		}
		if (!link || '_blank' === link.target || link.hasAttribute('download') || link.hasAttribute('data-mw-no-swap')) {
			return false;
		}
		// The catalog block owns its own in-place pagination and history.
		if (link.closest('.mw-catalog-results, .wp-block-query-pagination, form.mw-catalog-filters')) {
			return false;
		}
		var href = link.getAttribute('href');
		if (!href || /^(#|mailto:|tel:|javascript:|blob:|data:)/i.test(href)) {
			return false;
		}
		var url;
		try {
	...
	function navActivateScripts(root) {
		root.querySelectorAll('script').forEach(function (existing) {
			...
			for (var index = 0; index < attributes.length; index += 1) {
				replacement.setAttribute(attributes[index].name, attributes[index].value);
			}
	...
	function navUpdateHead(fresh) {
		...
		pairs.forEach(function (pair) {
			...
			if (freshNode && currentNode) {
				currentNode.setAttribute(pair[1], freshNode.getAttribute(pair[1]) || '');
			}
		});
	}
	...
	function navNavigate(url, push, scrollY) {
		...
		if (push) {
			history.replaceState(navMergeState({mwScrollY: window.scrollY}), '');
		}
		var fallback = function () {
			window.location.assign(url);
		};
		window.fetch(url, {credentials: 'same-origin', redirect: 'follow', headers: {'X-MW-Nav': '1'}})
			...
			.then(function (html) {
				var fresh = new window.DOMParser().parseFromString(html, 'text/html');
	...
	document.addEventListener('click', function (event) {
		...
		event.preventDefault();
		navNavigate(new window.URL(link.getAttribute('href'), window.location.href).href, true, 0);
	}, true);
	...
	window.addEventListener('popstate', function (event) {
		...
		if (event.state && event.state.mwNav) {
			navNavigate(window.location.href, false, event.state.mwScrollY || 0);
			return;
		}
		// History entries owned by the catalog filter block stay in place;
		// anything unrecognized after a swap falls back to a clean reload.
		if (event.state && event.state.mwCatalog && document.querySelector('form.mw-catalog-filters')) {
			return;
		}
