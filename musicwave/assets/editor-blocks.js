	...
	function sliderInspector( props ) {
		...
		return createElement(
			...
			createElement(
				...
				createElement( components.SelectControl, {
					label: __( 'اندازه تصویر', 'musicwave' ),
					value: props.attributes.imageSize || 'medium',
					options: [
						{ label: __( 'کوچک', 'musicwave' ), value: 'small' },
						{ label: __( 'متوسط', 'musicwave' ), value: 'medium' },
						{ label: __( 'بزرگ', 'musicwave' ), value: 'large' },
					],
					onChange( value ) {
			...
			createElement(
				components.PanelBody,
				{
					title: __( 'پیوند مشاهده همه', 'musicwave' ),
					initialOpen: false,
				},
				createElement( blockEditor.URLInput, {
					...
					onChange( value ) {
						props.setAttributes( { sectionUrl: value } );
					},
				} ),
				createElement( components.TextControl, {
					...
					onChange( value ) {
						props.setAttributes( { sectionLinkLabel: value } );
					},
				} )
			),
			createElement(
				components.PanelBody,
				{
					title: __( 'تب‌های فیلتر', 'musicwave' ),
					initialOpen: false,
				},
				createElement( components.TextareaControl, {
					label: __( 'تب‌ها', 'musicwave' ),
					help: __(
						'هر خط یک تب: امروز|orderBy:date یا ایندیه کره|taxonomy:mw_genre:korean-indie',
						'musicwave'
					),
					value: props.attributes.filterTabs || '',
					onChange( value ) {
						props.setAttributes( { filterTabs: value } );
					},
				} )
			),
			createElement(
