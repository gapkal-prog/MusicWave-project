 ...
 * @return string
 */
function musicwave_render_shelf_header( string $eyebrow, string $title, string $description = '', string $section_url = '', string $section_link_label = '', string $tabs = '' ): string {
	if ( '' === $eyebrow && '' === $title && '' === $description && '' === $section_url && '' === $tabs ) {
		return '';
	}
	...
	$link               = '' !== $section_url
		...
		: '';
	$tools              = ( '' !== $tabs || '' !== $link )
		? '<div class="mw-release-shelf__tools">' . $tabs . $link . '</div>'
		: '';

	return '<header class="mw-release-shelf__header"><div class="mw-release-shelf__heading">'
...
function musicwave_render_release_shelf( array $attributes ): string {
	...
	$header      = musicwave_render_shelf_header( $eyebrow, $section_title, $description, $section_url, $section_link_label, $tabs_markup );

	$nav = musicwave_shelf_nav_markup( $layout );

	return '<section ' . get_block_wrapper_attributes( array( 'class' => 'mw-release-shelf mw-release-shelf--' . $layout . ' mw-release-shelf--columns-' . $columns . $shelf_variant ) ) . '>' . $header . $nav . '<div class="mw-release-shelf__items" data-mw-shelf-viewport>' . implode( '', $cards ) . '</div></section>';
}
