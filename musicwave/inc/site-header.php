 ...
 *
 * Capability and nonce checks live here, not only at the caller, so a future
 * hook cannot persist $_POST without proving the request is authentic.
 *
 * @return bool Whether the mods were saved.
 */
function musicwave_save_header_admin_mods(): bool {
	if ( ! current_user_can( 'edit_theme_options' ) ) {
		return false;
	}

	check_admin_referer( 'musicwave_site_header' );

	$header = '';
	if ( isset( $_POST['musicwave_site_header'] ) && is_scalar( $_POST['musicwave_site_header'] ) ) {
		$header = sanitize_text_field( wp_unslash( (string) $_POST['musicwave_site_header'] ) );
	}

	$mobile = 'drawer';
	if ( isset( $_POST['musicwave_site_header_mobile'] ) && is_scalar( $_POST['musicwave_site_header_mobile'] ) ) {
		$mobile = sanitize_text_field( wp_unslash( (string) $_POST['musicwave_site_header_mobile'] ) );
	}

	$brand = 'full';
	if ( isset( $_POST['musicwave_topbar_brand'] ) && is_scalar( $_POST['musicwave_topbar_brand'] ) ) {
		$brand = sanitize_text_field( wp_unslash( (string) $_POST['musicwave_topbar_brand'] ) );
	}

	$tabs = '5';
	if ( isset( $_POST['musicwave_tabs_count'] ) && is_scalar( $_POST['musicwave_tabs_count'] ) ) {
		$tabs = sanitize_text_field( wp_unslash( (string) $_POST['musicwave_tabs_count'] ) );
	}

	set_theme_mod( 'musicwave_site_header', musicwave_sanitize_site_header( $header ) );
	set_theme_mod( 'musicwave_site_header_mobile', musicwave_sanitize_mobile_header( $mobile ) );
	set_theme_mod( 'musicwave_topbar_brand', musicwave_sanitize_topbar_brand( $brand ) );
	set_theme_mod( 'musicwave_rail_brand', musicwave_sanitize_onoff( isset( $_POST['musicwave_rail_brand'] ) ) );
	set_theme_mod( 'musicwave_show_search', musicwave_sanitize_onoff( isset( $_POST['musicwave_show_search'] ) ) );
	set_theme_mod( 'musicwave_show_theme', musicwave_sanitize_onoff( isset( $_POST['musicwave_show_theme'] ) ) );
	set_theme_mod( 'musicwave_show_account', musicwave_sanitize_onoff( isset( $_POST['musicwave_show_account'] ) ) );
	set_theme_mod( 'musicwave_tabs_count', musicwave_sanitize_tabs_count( $tabs ) );
...
function musicwave_render_header_admin_page(): void {
	...
	if ( 'POST' === $method && musicwave_save_header_admin_mods() ) {
